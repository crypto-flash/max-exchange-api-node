...
class Request {
    ...
    getPrivate(path, query = {}, querySchema = {}) {
        ...
        const body = this._decorate(query, path)
        const uri = this._fullUri(path, body)
    ...
    _fullUri(path, query = {}) {
        ...
        if (Object.keys(query).length > 0) {
            uri += `?${qs.stringify(decamelizeKeys(query), { arrayFormat: 'brackets' })}`
        }
    ...
    _decorate(body = {}, path = '') {
        ...
        if (nonce <= this._currentNonce) nonce = this._currentNonce + 1
